...
function showModal(url = '',tipe_modal = 'myModalLg') {

	$('#'+tipe_modal).find(".modal-content").html('<div class="modal-loading"><i class="fa fa-spinner fa-spin"></i><h3>Loading..</h3></div>');
    ...
    $.getJSON(url, function(data, status) {
        ...
        if (status == 'success') {
            ...
            $(modal_id).find(".modal-content").html(error_page("Ada kegagalan koneksi server. Silahkan hubungi Supervisor Area Anda"));
...
function showModalData(url = '', tipe_modal = 'myModalLg', data_ = Array()) {
    $('#' + tipe_modal).find(".modal-content").html('<div class="modal-loading"><i class="fa fa-spinner fa-spin"></i><h3>Loading..</h3></div>');
    var date = new Date();
    $('#' + tipe_modal).modal('show');
    $.ajax({
...
$(document).ready(function() {
    ...
    */
	...
	function _encrypt(data) {
		var splitz = data.split('&');
		var count = splitz.length;
		var string = '';
		var _dta = '';
		for(i=0;i < count;i++){        
			string = splitz[i].split('=');
			if (_dta == '') {
				if (string[0] == 'kd_dealer') {
					_dta = string[0]+'='+btoa(string[1]).replace('=','')
				} else {
					_dta = string[0]+'='+string[1]
				}
			} else {
				if (string[0] == 'kd_dealer') {
					_dta = _dta+'&'+string[0]+'='+btoa(string[1]).replace('=','')
				} else {
					_dta = _dta+'&'+string[0]+'='+string[1]
				}
			}        
		}
		return _dta;		
...
function onlyOnePage(url,notif) {
...
function valid_nama_customer(that){
	var id = $(that).attr('id');		

	if ($('#'+id).val() != '') {
		var value = $('#'+id).val()

		var validNama = /^([a-zA-Z',. ]{0,})$/;
		if (validNama.test(value)) {
			...
			$('.error').html("Karakter yang di perbolehkan hanya A-Z, tanda titik, koma, dan petik ( . , ' )").fadeIn();
			setTimeout(function() {
				hideAllMessages();
			}, 4000);
		...
		return validNama.test(value);

	 }	
}
function nBulan(mon) {
    return ['Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September','Oktober', 'November', 'Desember'][mon - 1];
 }
function validateAddress(str = '') {
    let status = false;

    let alamat = str.toString().toLowerCase();
    
    if (alamat != '') {
        let validate = [' rt.', ' rt', ' rt ', ' rt/', ' rw.', ' rw', ' rw ', ' rw/'];

        $.each(validate,function(index,data){

            let val = alamat.includes(data);
            ...
            if (val) {
                ...
                $('.error').html('Alamat Customer tidak boleh ada mengandung RT/RW').fadeIn();
        ...
        alert('Alamat customer tidak boleh kosong!');
